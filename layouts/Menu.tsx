  ...
  return (
    ...
    <div onClick={(e)=>handleMenu(e)} ref={menuEle} className={`${style.menu}`}>
        <div onClick={stopProp} id="content" className={`bg-slate-300 dark:bg-neutral-900 ${style.menu__content}`}>
            ...
            <div id="links" className={style.menuLinks}>
                <Link  href="/" >
                    <p onClick={handleMenu} className={`test block text-slate-700 dark:text-slate-300 ${style.hamburger__text} ${style.link}`}>HOME</p>
                </Link>
                <Link href="/projects">
                    <p onClick={handleMenu} className={`test block text-slate-700 dark:text-slate-300 ${style.hamburger__text} ${style.link}`}>PROJECTS</p>
                </Link>
                <Link href="/about">
                    <p onClick={handleMenu} className={`test block text-slate-700 dark:text-slate-300 ${style.hamburger__text} ${style.link}`}>ABOUT</p>
                </Link>
                <Link href="/contact" >
                    <p onClick={handleMenu} className={`test block text-slate-700 dark:text-slate-300 ${style.hamburger__text} ${style.link}`}>CONTACT</p>
                </Link>
            </div>
        </div>
